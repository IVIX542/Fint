...
export default async function () {
    ...
    const init = async () => {
        ...
        if (typeof Chart === 'undefined') {
            console.error('Chart.js not loaded. Stats view cannot render charts.');
            document.querySelector('.stats-view').innerHTML = `
                <div class="card" style="padding: 20px; text-align: center;">
                    <h3 style="color: var(--danger);">Error al cargar estadísticas</h3>
                    <p style="color: var(--text-muted);">No se pudo cargar la librería de gráficos.</p>
                    <p style="color: var(--text-muted); font-size: 0.85rem;">Asegúrate de tener conexión a internet al cargar la aplicación por primera vez.</p>
                </div>
            `;
            return;
        }
