    ...
    const init = async () => {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // 2. IMMEDIATE CLOUD SYNC: Upload to Supabase RIGHT NOW if online
                if (navigator.onLine) {
                    try {
                        const { supabase } = await import('../supabase.js');
                        if (supabase) {
                            const { data: { session } } = await supabase.auth.getSession();
                            if (session) {
                                // Prepare payload for Supabase (remove local-only fields)
                                const cloudPayload = { ...newTx, user_id: session.user.id };
                                delete cloudPayload.is_synced;
                                delete cloudPayload.categoryIcon;

                                // Upload immediately
                                console.log('🚀 Uploading transaction to cloud immediately...');
                                const { error } = await supabase.from('transactions').insert(cloudPayload);
                                ...
                                if (error) {
                                    ...
                                    newTx.is_synced = 1;
                                    await dbActions.add('transactions', newTx);
                                    // Realtime will notify other sessions automatically
                                }
                            } else {
                                // No session, queue for later
                                await dbActions.queueSyncAction({
                                    table: 'transactions',
                                    type: 'INSERT',
                                    payload: newTx
