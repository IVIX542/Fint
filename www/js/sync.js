...
export const syncManager = {
    ...
    async initRealtimeSubscription() {
        if (!supabase) {
            console.warn('⚠️ SyncManager: Supabase client not available - skipping Realtime');
            return;
        }

        const { data: { session } } = await supabase.auth.getSession();
        if (!session) {
            console.warn('⚠️ SyncManager: No active session - skipping Realtime');
            return;
        }

        // Don't subscribe if offline
        if (!navigator.onLine) {
            console.log('⚠️ SyncManager: Skipping Realtime - offline');
            return;
        }

        // Unsubscribe existing channel if any
        if (this.realtimeChannel) {
            console.log('SyncManager: Removing existing Realtime subscription');
        ...
        // Use unique channel name with session ID to avoid conflicts
        const channelName = `db-changes-${session.user.id}`;
        ...
        this.realtimeChannel = supabase
            ...
            .on(
                'postgres_changes',
                {
                    ...
                    schema: 'public',
                    table: 'transactions',
                    filter: `user_id=eq.${session.user.id}` // RLS filter on client side too
                },
                async (payload) => {
                    console.log('🔔 Realtime Change (Transactions):', payload.eventType, payload);
                    ...
                    if (payload.eventType === 'INSERT' && payload.new) {
                        ...
                        const item = { ...payload.new, is_synced: 1 };
                        await dbActions.add('transactions', item); // Overwrites existing
                        window.dispatchEvent(new Event('fint-data-changed'));
                    } else if (payload.eventType === 'DELETE' && payload.old) {
                        await dbActions.delete('transactions', payload.old.id);
                        window.dispatchEvent(new Event('fint-data-changed'));
                    }
                }
            )
            .on(
                ...
                async (payload) => {
                    console.log('🔔 Realtime Change (Categories):', payload.eventType, payload);
                    ...
                    if (payload.eventType === 'INSERT' && payload.new) {
                        ...
                        await dbActions.delete('categories', payload.old.id);
                        window.dispatchEvent(new Event('fint-data-changed'));
                    }
                }
            )
    ...
    async stopRealtimeSubscription() {
        if (this.realtimeChannel) {
            console.log('SyncManager: Stopping Realtime subscription');
            await supabase.removeChannel(this.realtimeChannel);
            this.realtimeChannel = null;
        }
    }
